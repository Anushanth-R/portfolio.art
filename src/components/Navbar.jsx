...
function Navbar() {
  ...
  return (
    <header id="navbar" className="bg-[var(--bg2-color)] flex justify-between items-center w-full h-[40px] px-[20px] border-b border-[var(--primary-color)] shadow-[0_0_10px_var(--primary-color)] sticky top-0">
      ...
      {/* Nav Links */}
      <nav>
        <ul id="nav-links" className={`flex flex-col md:flex-row items-end absolute md:relative gap-3 py-2 md:p-0 rounded-[10px] shadow-[0_0_10px_var(--primary-color)] md:shadow-none bg-[var(--bg2-color)] md:gap-7 top-[42px] right-[2px] md:top-0 md:right-0 text-base lg:text-lg text-[var(--primary-color)] font-medium transition-all duration-300 ease-in-out ${showNav ? 'opacity-100 scale-100' : 'opacity-0 scale-80 pointer-events-none'} md:opacity-100 md:scale-100 md:pointer-events-auto`}>
